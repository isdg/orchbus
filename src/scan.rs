...
//!
//! Cache row (6 fields): rank <TAB> pane_id <TAB> glyph <TAB> session:win <TAB> topic <TAB> question
//! List row  (5 fields): pane_id <TAB> glyph <TAB> session:win <TAB> topic <TAB> question
//!
//! pane_id (e.g. %23) is the sole tmux target the UI uses; fields 2.. are display
//! only (the UI hides field 1 from fzf matching with --with-nth=2..).
...
impl Row {
    fn cache_line(&self) -> String {
        format!(
            "{}\t{}\t{}\t{}\t{}\t{}",
            self.rank, self.pid, self.glyph, self.swin, self.title, self.question
        )
    }
    fn list_line(&self) -> String {
        format!(
            "{}\t{}\t{}\t{}\t{}",
            self.pid, self.glyph, self.swin, self.title, self.question
        )
    }
...
/// Build a row for one pane, or `None` if it isn't a live CC pane worth showing.
fn scan_pane(pid: &str, swin: &str, title: &str) -> Option<Row> {
...
/// Scan every CC pane across all sessions.
fn full() -> Result<String> {
    ...
    let rows: Vec<Row> = panes
        ...
        .filter_map(|line| {
            ...
            if f.len() == 4 && f[1] == "claude" {
                scan_pane(f[0], f[2], f[3])
            } else {
                None
            }
        })
...
fn splice(pid: &str) -> Result<String> {
    ...
    // Add this pane's fresh row if it's still a live CC pane.
    let panes = list_panes()?;
    if let Some(line) = panes.lines().find(|l| l.starts_with(&format!("{pid}\t"))) {
        let f: Vec<&str> = line.splitn(4, '\t').collect();
        if f.len() == 4 && f[1] == "claude" {
            if let Some(row) = scan_pane(f[0], f[2], f[3]) {
                rows.push(row);
            }
        }
    }
...
mod tests {
    ...
    fn cache_line_round_trips_and_list_drops_rank() {
        ...
        assert_eq!(r.list_line(), "%3\t[!]\ts:1\ttopic\tproceed?");
    }
